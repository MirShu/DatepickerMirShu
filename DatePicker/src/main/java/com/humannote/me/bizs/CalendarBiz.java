...
import com.humannote.me.entities.Gregorian;
import com.humannote.me.entities.Lunar;
...
public class CalendarBiz implements IBO {
    ...
    private static final String[] LUNAR_FESTIVAL = {"春节 ", "元宵节 ", "端午节 ", "七夕 ", "中秋节 ", "重阳节 ", "腊八节 "};
...
//        return a;
        ...
        int lastMonthDay = lastMonth.getActualMaximum(Calendar.DAY_OF_MONTH) - firstDayOfWeek + 1;
        String a[][] = new String[6][7];
        int nextDay = 1;
        for (int i = 0; i < 6; i++) {
            for (int j = 0; j < 7; j++) {
                if (j < firstDayOfWeek && nextDay == 1) {
                    a[i][j] = String.valueOf(lastMonthDay++);
                } else if (nextDay > calendar.getActualMaximum(Calendar.DAY_OF_MONTH)) {
                    a[i][j] = String.valueOf(nextMonthDay++);
                } else {
                    a[i][j] = String.valueOf(nextDay++);
                }
            }
        }
        return a;
